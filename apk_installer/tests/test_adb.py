...
from apk_installer.adb import check_adb_exists, get_adb_version, get_devices
...
def test_get_adb_version_failure():
    with patch(
        "subprocess.check_output", side_effect=subprocess.CalledProcessError(1, "adb")
    ):
...
def test_get_devices_success():
    mock_output = (
        ...
        "model:sdk_gphone64_arm64 device:emulator64_arm64 transport_id:1\n"
        "0123456789ABCDEF       device usb:1-1 product:redfin "
        "model:Pixel_5 device:redfin transport_id:2\n"
    )
    with patch("subprocess.check_output", return_value=mock_output):
        devices = get_devices()
